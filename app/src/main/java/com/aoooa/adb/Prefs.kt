...
object Prefs {
    ...
    private const val NAME = "aoooa_adb_prefs"
    private const val OLD_NAME = "webadb_prefs"
    ...
    /** 兼容迁移旧版本 SharedPreferences 配置并在完成后清理旧文件 */
    private fun migrateFromOldPrefs(context: Context) {
        try {
            val prefsDir = File(context.filesDir.parentFile, "shared_prefs")
            val oldPrefsFile = File(prefsDir, "$OLD_NAME.xml")
            if (!oldPrefsFile.exists() || oldPrefsFile.length() <= 0) return

            val oldSp = context.getSharedPreferences(OLD_NAME, Context.MODE_PRIVATE)
            val allOld = oldSp.all
            if (allOld.isNotEmpty() && sp.all.isEmpty()) {
                val editor = sp.edit()
                for ((k, v) in allOld) {
                    when (v) {
    ...
    fun loadCommands(): List<com.aoooa.adb.model.CommandItem> {
        ...
        return try {
            ...
            val list = mutableListOf<com.aoooa.adb.model.CommandItem>()
    ...
    /** 保存快捷指令列表 */
    fun saveCommands(list: List<com.aoooa.adb.model.CommandItem>) {
        try {
    ...
    /** 恢复默认预设指令 */
    fun resetDefaultCommands(): List<com.aoooa.adb.model.CommandItem> {
    ...
    /** 默认预设指令列表 */
    fun getDefaultCommands(): List<com.aoooa.adb.model.CommandItem> {
        return listOf(
            com.aoooa.adb.model.CommandItem(
                id = "cmd_shizuku",
